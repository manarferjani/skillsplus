...
const WorstTechnologyCard = ({ testRecords }: WorstTechnologyCardProps) => {
  ...
  const getWorstTechnology = (name: string) => {
    const filtered: Record<
      string,
      { totalScore: number; totalRate: number; count: number }
    > = {}
  ...
  const displayedCollaborateur =
    selectedCollaborateur || allCollaborateurs[currentIndex] || ''
  const worstTech = getWorstTechnology(displayedCollaborateur)

  return (
    <Card className='relative !rounded-3xl border-0 shadow-md'>
      <CardHeader className='pb-0'>
        <div className='flex items-center justify-between'>
          ...
          <a
            href='/_authenticated/userProfile#badges'
