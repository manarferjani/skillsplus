...
router.post('/add', auth, async (req, res) => {
  try {
    const { title, level, technology, scheduledDate, duration, questions } = req.body;

    // Vérifier si tous les champs requis sont présents
    if (!title || !level || !technology || !scheduledDate || !duration || !questions?.length) {
      return res.status(400).json({ message: "Champs obligatoires manquants." });
    }

    // Loguer le nom de la technologie pour voir ce qui est envoyé
    console.log('Nom de la technologie recherché :', technology);

    // Recherche insensible à la casse
    const technologie = await Technologie.findOne({ name: new RegExp(`^${technology}$`, 'i') });

    // Loguer la technologie trouvée pour vérifier ce qui a été renvoyé
    console.log('Technologie trouvée :', technologie);

    if (!technologie) {
      return res.status(400).json({ message: `Technologie "${technology}" non trouvée.` });
    }

    // Préparer les données du test en ajoutant l'ObjectId de la technologie et l'ID de l'utilisateur
    const testData = {
      title,
      level,
      technology: technologie._id, // Remplacer le nom par l'ObjectId
      scheduledDate,
      duration,
      createdBy: req.userId, // Utiliser l'ID de l'utilisateur récupéré dans le token JWT
      questions
    };

    // Créer le test en utilisant le service d'ajout
    const newTest = await TestService.addTest(testData);
    res.status(201).json(newTest); // Retourner le test créé avec succès
  } catch (error) {
    console.error(error); // Loguer l'erreur pour mieux comprendre ce qui ne va pas
...
router.get('/:id/questions', async (req, res) => {
  try {
    ...
    console.error('Erreur dans la route /api/tests/:id/questions:', error.message)
    res.status(500).json({
      success: false,
      message: error.message || 'Erreur serveur lors de la récupération du test'
    })
  }
})
...
router.post('/:id/calculate-statistics', async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: error.message || "Erreur lors du calcul des statistiques du test"
    });
  }
});
