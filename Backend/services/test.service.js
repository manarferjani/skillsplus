...
class TestService {
  ...
  async getFormattedTests(userId = null) {
    try {
      // Récupération de tous les tests avec les jointures nécessaires
      const tests = await Test.find()
        .populate({
          path: "technology",
          select: "name",
      ...
      // Formatage des tests avec filtrage si userId est présent
      const formattedTests = tests
        .map((test) => {
          const technology = test.technology || {};
          const participations = test.participations || [];

          // Si un ID est donné, ne garder que les participations du collaborateur
          const filteredParticipations = userId
            ? participations.filter(
                (p) => p.user && p.user._id?.toString() === userId
              )
            : participations;

          // Si après filtrage, il n'y a plus de participations, on ignore ce test
          if (userId && filteredParticipations.length === 0) {
            return null;
          }
        ...
        .filter((test) => test !== null); // Élimine les tests vides s'il n'y a pas de participations pour le user
  ...
  async getTestByIdFormatted(id) {
    ...
    try {
      const test = await Test.findById(id)
        .select("+questions._id") // ⚠️ Nécessaire pour inclure les _id des questions
        .populate({
          path: "technology",
          select: "name",
          model: "Technologie",
      ...
      const formattedQuestions = test.questions.map((question) => {
        ...
        return {
          _id: question._id.toString(), // ✅ Conservez l'ID original
          id: question._id.toString(), // ✅ Double sécurité pour le frontend
      ...
      return {
        ...
        TestBasicScore: test.TestBasicScore ?? 0,
        TestIntermediateScore: test.TestIntermediateScore ?? 0,
        TestExpertScore: test.TestExpertScore ?? 0,
        TestMaxScore: test.TestMaxScore ?? 0,
      };
    } catch (error) {
  ...
  async getTestsForCalendar() {
    ...
    const tests = await Test.find({ 
      ...
      status: "scheduled"  // On récupère uniquement ceux dont le status est "scheduled"
    })
  ...
  async calculateTestStatistics(testId) {
    ...
    test.participations.forEach((participation) => {
      if (
        participation.totalScore !== null &&
        participation.successRate !== null
      ) {
