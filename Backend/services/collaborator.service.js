...
class CollaboratorService {
  ...
  async getPerformers({ filter = "all", limit, sortBy }) {
    try {
      ...
      // On ajoute `profileImage` dans `.select()` pour qu'il soit inclus
      const performers = await query
        .populate("performerOfTheWeek.technologyId", "name")
        .populate("technology_success_rate.technologyId", "name")
        .select(
          "name profileImage jobPosition technology_success_rate performerOfTheWeek"
        ) // ✅ profileImage ajouté ici
        .lean();

      return performers.map((p) => ({
        name: p.name,
        jobPosition: p.jobPosition,
        profileImage: p.profileImage || null, // ✅ Maintenant disponible
