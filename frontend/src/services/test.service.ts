...
/**
 * Récupère les tests formatés via l'endpoint GET /api/tests/getFormattedTests.
 * Si un userId est fourni, ne récupère que les tests de ce collaborateur.
 * 
 * @param {string | null} userId - ID du collaborateur à filtrer (optionnel)
 */

export async function fetchFormattedTests(userId: string | null = null) {
  try {
    const url = userId
      ? `/api/tests/getFormattedTests?userId=${encodeURIComponent(userId)}`
      : '/api/tests/getFormattedTests';

    const response = await apiClient.get(url, {
      timeout: 30000 // Surcharge locale du timeout
    });


    if (!response.data?.data) {
      throw new Error('Réponse inattendue du serveur: structure de données manquante');
    }

    console.log("Formatted tests récupérés :", response.data.data);
    ...
    console.error("Échec de fetchFormattedTests:", {
      error: errorMessage,
      endpoint: '/api/tests/getFormattedTests',
      userId,
      timestamp: new Date().toISOString()
    });

    throw new Error(`Impossible de charger les tests: ${errorMessage}`);
  }
}
