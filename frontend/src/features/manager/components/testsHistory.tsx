...
// ou le bon chemin vers ton hook d'auth

interface testsHistoryTableProps {
  testRecords: TestRecord[]
}

const testsHistoryTable: React.FC<testsHistoryTableProps> = ({
  testRecords = [],
}) => {
  ...
  function getDiagnosisColor(technology: string): string {
    ...
    console.log('Tech for color:', techKey)
    const colors: Record<string, string> = {
      'react.js': 'bg-yellow-100 text-yellow-600',
      angular: 'bg-red-100 text-red-600',
      'vue.js': 'bg-teal-100 text-teal-600',
      'express.js': 'bg-orange-100 text-orange-600',
      laravel: 'bg-purple-100 text-purple-600',
      'next.js': 'bg-emerald-100 text-emerald-600',
      flutter: 'bg-blue-100 text-blue-600',
      django: 'bg-fuchsia-100 text-fuchsia-600',
      'spring boot': 'bg-pink-100 text-pink-600',
    }
  ...
  return (
    <div className='p-4'>
      ...
      {/* Table Header */}
      <div className='mb-2 flex items-center text-xs font-semibold uppercase text-gray-500'>
          ...
          groupKeys.map((technology) =>
            groupedData[technology].map((item, index) => {
              ...
              return (
                <div
                  key={item.id}
                  className={`flex items-center border border-[#FAFAFA] bg-[#FAFAFA] p-1 text-[13px] text-gray-700 ${isFirst ? 'rounded-t-3xl' : ''} ${isLast ? 'rounded-b-3xl' : ''}`}
                >
                  <div className='w-1/6'>
                    {isFirst && (
                      <span
                        className={`inline-block rounded-full px-3 py-1 text-[10px] font-medium ${getDiagnosisColor(item.technology)}`}
                      >
                        {item.technology}
                      </span>
                    )}
