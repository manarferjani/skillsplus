...
const EditTestForm: React.FC<EditTestFormProps> = ({ testId }) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        console.log('Données complètes du test:', test)
        console.log('Technologies disponibles:', techs)

        // Gestion des technologies
        const technologiesArray = Array.isArray(techs)
          ? techs
          : Object.values(techs)
        setTechnologies(technologiesArray as Technology[])

        // Trouver la technologie correspondante
        const techId =
          typeof test.technology === 'object'
            ? test.technology._id
            : test.technology

        const matchedTech = technologiesArray.find((t) => t._id === techId)

        console.log('Technologie trouvée:', matchedTech, 'pour ID:', techId)

        setTitle(test.title)
        setLevel(test.level)
        setTechnologyId(matchedTech?._id || '')
        ...
        // Calcul des scores
        const calculatedMaxScore = test.questions.reduce(
    ...
    return () => {
      controller.abort()
      console.log('Nettoyage useEffect - requête annulée')
    }
  }, [testId, navigate])
  ...
  const handleAddQuestion = () => {
    ...
    setQuestions((prev) => [
      ...prev,
      {
        ...
        correctAnswers: [],
        level: 'basic',
        duration: 60, // ou une valeur par défaut
      },
    ])
  }
  ...
  // Ajoutez ces logs juste avant le return principal
  console.log('État actuel:', {
    technologyId,
    technologies,
    selectedTechnology: technologies.find((tech) => tech._id === technologyId),
    scheduledDate,
  })
              ...
              <div>
                ...
                <div className='relative'>
                  ...
                  >
                    ...
                    <option value='intermediate'>Intermédiair</option>
            ...
            <div className='grid grid-cols-1 gap-6 md:grid-cols-2'>
              <div>
                ...
                <div className='relative'>
                  <select
                    id='technology'
                    value={technologyId || ''} // Ajout d'une valeur par défaut
