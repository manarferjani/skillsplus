...
export function SignUpForm({ className, ...props }: SignUpFormProps) {
  ...
  const handleGoogleCallback = async (response: any) => {
    try {
      ...
      if (payload && payload.name && payload.email && payload.sub) {
        const result = await authAPI.signUpWithSocial(
          payload.name,
          payload.email,
          payload.sub,
          'google'
      ...
      setError(err.response?.data?.message || 'An error occurred during Google sign up');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleGitHubCallback = async (code: string) => {
    try {
      ...
      // Exchange the code for an access token and user info via backend
      try {
        // Call backend to handle the GitHub OAuth code exchange
        ...
        if (result.success) {
          ...
          setError(result.message || 'Failed to sign up with GitHub');
        }
      } catch (err: any) {
        console.error('GitHub sign-up error:', err);
        setError(err.response?.data?.message || 'An error occurred during GitHub sign up');
        
        // Fallback message
        toast({
          title: "GitHub Sign-Up",
          description: "GitHub sign-up requires backend implementation. Please add the exchange handler to your server.",
          variant: "destructive",
        });
      }
    } finally {
        ...
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <div className='grid gap-2'>
            <FormField
              ...
              render={({ field }) => (
                <FormItem className='space-y-1'>
                  <FormLabel className="text-white" style={{ fontFamily: "'Montserrat', sans-serif", fontWeight: 200 }}>Full name</FormLabel>
                  <FormControl>
                    <Input 
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className='space-y-1'>
                  <FormLabel className="text-white" style={{ fontFamily: "'Montserrat', sans-serif ", fontWeight: 200 }}>Email</FormLabel>
                  <FormControl>
                    <Input 
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className='space-y-1'>
                  <FormLabel className="text-white" style={{ fontFamily: "'Montserrat', sans-serif" , fontWeight: 200 }}>Password</FormLabel>
                  <FormControl>
                    <PasswordInput 
                      placeholder='********' 
                      {...field} 
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className='space-y-1'>
                  <FormLabel className="text-white" style={{ fontFamily: "'Montserrat', sans-serif", fontWeight: 200 }}>Confirm Password</FormLabel>
                  <FormControl>
                    <PasswordInput 
                      placeholder='********' 
                      {...field} 
            ...
            <div className='relative my-2'>
              ...
              <div className='relative flex justify-center text-xs uppercase'>
                <span className='bg-gray-800 px-2 text-gray-400' style={{ fontFamily: "'Montserrat', sans-serif" }}>
                  Or
                </span>
              </div>
            </div>
            ...
            <div className='flex justify-center gap-4 mt-2'>
              ...
              <button
                ...
                style={{ display: "none" }} // Hide this button since we're using the Google rendered button
              >
